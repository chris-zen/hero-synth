...
impl<'a> Decoder<'a> {
    pub fn new(data: &'a [u8]) -> Decoder<'a> {
        Decoder { pos: 0, start: 0, sysex_data: Vec::new(), sysex_decoding: false, data: data }
    }
    ...
    fn decode_note(&mut self, channel: u4, is_on: bool) -> Message {
        match self.next_data2() {
            Ok((key, velocity)) => match is_on {
                ...
                false => Message::NoteOff { channel: channel, key: key, velocity: velocity }
            },
    ...
    fn decode_control_change(&mut self, channel: u4) -> Message {
        match self.next_data2() {
            Ok((controller, value)) => match controller {
                ...
                126 => Message::MonoModeOn { channel: channel, num_channels: value },
                127 => match value {
    ...
    fn decode_mtc_quarter_frame(&mut self) -> Message {
        match self.next_data() {
            Ok(data) => Message::MTCQuarterFrame { msg_type: (data >> 4) & 0x07, value: data & 0x0f },
            Err(end) => self.unknown(end)
        }
    }
    ...
    fn decode_song_position_pointer(&mut self) -> Message {
        match self.next_data2() {
            Ok((lsb, msb)) => Message::SongPositionPointer { beats: (msb << 7) | lsb },
            Err(end) => self.unknown(end)
        }
    }
    ...
    fn decode_sysex_data(&mut self) -> Option<Message> {
        let start = self.pos;
        let mut pos = start;
        while pos < self.data.len() && (self.data[pos] & 0x80) == 0 { pos += 1 }
        self.pos = pos;
...
mod tests {
    ...
    fn decode_notes() {
        ...
        let mut dec = Decoder::new(data);
        assert_eq!(dec.next(), Some(Message::NoteOff { channel: 0b0101, key: 64, velocity: 127 }));
        assert_eq!(dec.next(), Some(Message::NoteOn { channel: 0b1010, key: 0, velocity: 127 }));
        assert_eq!(dec.next(), None);
    }
    ...
    fn decode_mtc_quarter_frame() {
        ...
        let mut dec = Decoder::new(data);
        assert_eq!(dec.next(), Some(Message::MTCQuarterFrame { msg_type: 0b101, value: 0b1010 }));
        assert_eq!(dec.next(), None);
    }

    #[test]
    fn decode_song_position_pointer() {
        let data = &vec![0b1111_0010u8, 0b0_1010101, 0b0_0101010, 0b1111_0010u8, 0b0_0101010, 0b0_1010101];
        let mut dec = Decoder::new(data);
        assert_eq!(dec.next(), Some(Message::SongPositionPointer { beats: 0b01010101010101 }));
        assert_eq!(dec.next(), Some(Message::SongPositionPointer { beats: 0b10101010101010 }));
        assert_eq!(dec.next(), None);
    }
    ...
    fn decode_sysex_continuous() {
        ...
        let mut dec = Decoder::new(data);
        assert_eq!(dec.next(), Some(Message::SysEx { data: vec![1u8, 2, 3, 4] }));
        assert_eq!(dec.next(), None);
    }

    #[test]
    fn decode_sysex_interleaved() {
        let data = &vec![0b1111_0000u8, 1, 2, 0b1000_0101u8, 64, 127, 3, 4, 0b1111_0111];
        let mut dec = Decoder::new(data);
        assert_eq!(dec.next(), Some(Message::NoteOff { channel: 0b0101, key: 64, velocity: 127 }));
        assert_eq!(dec.next(), Some(Message::SysEx { data: vec![1u8, 2, 3, 4] }));
        assert_eq!(dec.next(), None);
    ...
    fn decode_sysex_unexpected_end_data() {
        ...
        let mut dec = Decoder::new(data);
        assert_eq!(dec.next(), Some(Message::Unknown(vec![0b1111_0000u8, 1, 2])));
        assert_eq!(dec.next(), None);
    }
